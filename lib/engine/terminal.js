      ...
      if (!query) {
        return {
          ...
          message: `${verb === "buy" ? "Buy" : "Sell"} what? Try \`${
            verb === "buy" ? "buy $5 nvda" : "sell all nvda"
          }\`.`,
        };
      }
      if (!taken) {
    ...
    case "audit": {
      ...
      if (!query) {
        return {
          kind: "error",
          raw,
          message: `Which token? Try \`${verb} nvda\` or paste a contract address.`,
        };
      }
...
const HELP = [
  ["buy $5 nvda", "Buy five dollars of a stock token or launch. Bare numbers are dollars."],
  ["buy 0.01 eth nvda", "Name the unit to spend ETH instead."],
  ["sell all nvda", "Sell the whole position. `sell 50% nvda` and `sell 1000 nvda` work too."],
  ["price nvda", "Live price and market cap — stock tokens and launches alike."],
  ["audit 0x…", "Full honeypot and owner-power scan."],
  ["fund", "Show your wallet address to top it up with ETH."],
  ["send 0.05 eth to 0x…", "Move ETH out. `send 100 nvda to 0x…` moves a token."],
  ["convert $100 to eth", "Quick price math at the live rate."],
  ["gas", "Current gas price and chain status."],
  ["create TICKER Your Token", "Open the create form, ticker and name filled in."],
