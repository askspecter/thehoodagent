      ...
      if (!query) {
        return {
          ...
          message: `${verb === "buy" ? "Buy" : "Sell"} what? Try \`${
            verb === "buy" ? "buy $5 pons" : "sell all pons"
          }\`.`,
        };
      }
      if (!taken) {
    ...
    case "audit": {
      ...
      if (!query) {
        return {
          kind: "error",
          raw,
          message: `Which token? Try \`${verb} pons\` or paste a contract address.`,
        };
      }
...
const HELP = [
  ["buy $5 pons", "Buy five dollars of $PONS. Bare numbers are dollars."],
  ...
  ["price pons", "Live pool price, market cap and graduation progress."],
  ["audit 0x…", "Full honeypot and owner-power scan."],
  ["create HOOD Hood Rat", "Open the create form, ticker and name filled in."],
