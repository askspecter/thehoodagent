...
/**
 * Read for pricing, not for display. The launch record's `supply` is the amount
 * the factory minted; what a market cap needs is what exists *now*, which is
 * `totalSupply()` — they differ the moment anyone burns.
 */
...
const Q192 = 1n << 192n;
/** Fixed-point headroom. 1e36 keeps ~36 digits below the point before rounding. */
const FIXED = 10n ** 36n;
 ...
 *
 *  1. It squares in BigInt. `Number(sqrtPriceX96) / 2**96` rounds to 53 bits
 *     *before* squaring, which doubles the relative error and, for the very
 *     small prices a fresh 1e9-supply launch trades at, was the difference
 *     between a believable price and a wrong one.
 *  2. It scales by the decimal difference. A token that does not use 18 decimals
 *     was previously mispriced by a factor of 10^(18-d) — silently, because the
 *     result still looked like a number.
 *
 * @param {bigint} sqrtPriceX96 pool `slot0().sqrtPriceX96`
 * @param {boolean} isToken0    true when the launch token is the pool's token0
 * @param {number} tokenDecimals  launch token decimals (default 18)
 * @param {number} wethDecimals   paired WETH decimals (default 18)
 * @returns {number|null} WETH per whole token, or null if the pool has no price
 */
function priceFromSqrt(sqrtPriceX96, isToken0, tokenDecimals = 18, wethDecimals = 18) {
  const sqrt = BigInt(sqrtPriceX96 ?? 0);
  if (sqrt <= 0n) return null;

  // token1 raw units per 1 raw unit of token0, held as a 1e36 fixed-point int.
  const rawRatioFixed = (sqrt * sqrt * FIXED) / Q192;
  if (rawRatioFixed === 0n) return null;

  const rawRatio = Number(rawRatioFixed) / 1e36;
  if (!Number.isFinite(rawRatio) || rawRatio === 0) return null;

  // Reading the pool from the other side is exactly the reciprocal; the decimal
  // adjustment is the same either way, because it only ever converts
  // "raw units of the token per raw unit of WETH" into whole-unit terms.
  const base = isToken0 ? rawRatio : 1 / rawRatio;
  const scaled = base * 10 ** (Number(tokenDecimals) - Number(wethDecimals));
...
async function readLaunch(provider, chain, token) {
  ...
  // ---- Live pool price ----
  //
  // Decimals and total supply are read from the token itself rather than assumed.
  // The factory's `supply` is what was minted at launch; `totalSupply()` is what
  // exists now, and a burn makes those two different numbers. Using the wrong one
  // overstates the market cap by exactly the burned amount.
  const erc20 = new Contract(token, ERC20_SUPPLY_ABI, provider);
  const [decimalsRaw, totalSupplyRaw] = await Promise.all([
    attempt(() => erc20.decimals(), null),
    attempt(() => erc20.totalSupply(), null),
  ]);

  out.decimals = decimalsRaw === null ? 18 : Number(decimalsRaw);
  out.totalSupply = totalSupplyRaw === null ? out.supply : totalSupplyRaw;
  out.supplyBurned = totalSupplyRaw !== null && out.supply != null && totalSupplyRaw < out.supply;
  ...
  if (out.pool && out.pool !== ZeroAddress) {
    ...
    if (slot0) {
      const priceInWeth = priceFromSqrt(slot0[0], out.isToken0, out.decimals, 18);
      if (priceInWeth !== null) {
        out.priceInWeth = priceInWeth;
        out.sqrtPriceX96 = slot0[0].toString();

        const supplyTokens = Number(formatUnits(out.totalSupply ?? 0n, out.decimals));
        if (Number.isFinite(supplyTokens) && supplyTokens > 0) {
          out.supplyTokens = supplyTokens;
          // Nothing here vests or unlocks, so circulating supply is total supply
          // and market cap and FDV are the same figure.
