...
function secret() {
  ...
  if (!value || value.length < 32) {
    throw new Error(
      "SESSION_SECRET is missing or too short (need 32+ characters). Generate one with:\n" +
        "  node -e \"console.log(require('crypto').randomBytes(32).toString('hex'))\"\n" +
        "and set it in .env.local locally, or under Settings → Environment Variables on Vercel."
    );
  }
  return value;
}
